...
Deep research note for the UltraFast whitepaper, covering Intel Trust Domain Extensions (TDX) and AMD Secure Encrypted Virtualization with Secure Nested Paging (SEV-SNP) — the two confidential-computing technologies that back the dark-pool tier of UltraFast's privacy framework.
...
**Phala Network (TDX, GPU TEE).** Phala originally ran on SGX. In December 2024, in response to Intel's discontinuation of SGX-IAS and to subsequent SGX vulnerabilities (notably WireTap, see below), Phala announced sunsetting Khala chain workers, migrating to Intel TDX as the host-side TEE, and adding NVIDIA Confidential Computing for GPU-side workloads. Phala's DStack tool packages TDX confidential VMs for application developers; Phala 2.0 (2025) carries this forward with an Op-Succinct L2 on Ethereum.
