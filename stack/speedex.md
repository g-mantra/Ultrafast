...
3. **Commutative execution engine ("Groundhog").** Once prices are set, the per-order fills can be applied in any order - the semantics are commutative. Speedex exploits this for parallel settlement on multi-core hardware. The follow-on paper "Groundhog: Linearly-Scalable Smart Contracting via Commutative Transaction Semantics" (arXiv:2404.03201, 2024) generalises the commutative-execution model to arbitrary smart contracts. It is positioned as an alternative to software transactional memory (STM) for blockchain VMs.
