...
Research notes on CGGMP21 - "UC Non-Interactive, Proactive, Threshold ECDSA with Identifiable Aborts". The UltraFast whitepaper references it as the ECDSA fallback TSS protocol in the §10.1 working-assumption table, and as a sizing input in §13.3 and §13.4. Compiled for internal use during the pre-implementation research phase.
...
CGGMP21 appears in UltraFast as the **ECDSA fallback** for secp256k1 corridors. These corridors include Bitcoin (legacy and SegWit), Ethereum L1, and every EVM L2. It sits alongside DKLs23, the working-assumption primary ECDSA protocol per the §10.1 TSS-per-curve table.

The fallback designation has a specific meaning. If Phase-0 benchmarking or audit review surfaces a problem with the chosen DKLs23 implementation (`silence-laboratories/dkls23`), CGGMP21 - via `LFDT-Lockness/cggmp21` - is the pre-vetted swap-in. The same applies if a future implementation-class incident takes that library out of the trusted set. The two are listed in the same table row family for that reason: same curve, same threshold-ECDSA problem, different cryptographic constructions and different operational profiles.

CGGMP21 also defines the upper bound on the §13.3 validator-set sizing exercise. The whitepaper records "FROST sign at $n = 100$ measures at ~150–300 ms over WAN; DKG at $n = 100$ measures at a few seconds for DKLs23 and **tens of seconds for CGGMP21**."

The DKG cost ratio between the two ECDSA protocols at $n=100$ is the single biggest reason the table entries are not interchangeable. Running CGGMP21 as primary would extend the epoch boundary at which UltraFast generates a new TSS wallet on each foreign chain (§10.3 fresh-wallet rotation) from seconds to a minute-class operation. That forces a wider sweep window and a larger stale-wallet attack surface.
...
2. **UC proof and proactive secret sharing built in.** CGGMP21 is universally composable under standard assumptions (strong RSA, DDH, Paillier), withstands adaptive corruption, and ships proactive refresh in the same protocol. This matches UltraFast's §10.3 fresh-wallet rotation model and the §13.4 slashing schedule's assumption that share refresh can run on a cadence independent of DKG.
...
CGGMP21 is usually deployed by custody platforms (Fireblocks's MPC-CMP, Dfns, Taurus) as the **primary** ECDSA scheme in a 2-of-3 or small-$n$ configuration. That is the regime where Paillier overhead is dominated by network latency and DKG cost is a one-off. UltraFast's regime is the opposite: $n \approx 30$ at launch with a milestone path to ~100. The system runs frequent epoch-bounded re-DKGs against multiple foreign chains, with a hard latency target on the bridge signing path. That changes the constants enough that DKLs23 wins the working-assumption slot and CGGMP21 takes the fallback slot.
...
CGGMP21 is the protocol introduced by Ran Canetti, Rosario Gennaro, Steven Goldfeder, Nikolaos Makriyannis, and Udi Peled in "UC Non-Interactive, Proactive, Threshold ECDSA with Identifiable Aborts", ACM CCS 2020 (DOI: 10.1145/3372297.3423367). The full version is Cryptology ePrint Archive Paper 2021/060. That version is the canonical reference because the conference version is condensed, and several of the proofs and the proactive-refresh construction only appear in full there. The naming convention follows the authors' surnames plus the year of the full ePrint version. The implementation ecosystem has settled on that convention; the conference version is sometimes called "CGGMP" without a year.
...
1. **HD-wallet derivation-path malleability.** `Presignature::set_derivation_path` allowed the SLIP-10 derivation path to be changed after presignature creation. A malicious participant could exploit this to weaken the effective security level of derived keys.
2. **Raw-hash signing → signature forgery.** `Presignature::issue_partial_signature` accepted a pre-hashed message without binding to the original message. That allowed an attacker to craft a scalar that produces a valid signature on a different message of their choice.
...
**TSSHOCK class.** The Verichains TSSHOCK disclosures at Black Hat USA 2023 documented key-extraction attacks across many `tss-lib` derivatives implementing GG18/GG20/CGGMP21. The whitepaper's §10.1 statement that GG18/GG20 are excluded is direct: those families are too compromised to ship. CGGMP21 implementations have generally been patched against the TSSHOCK class. The lesson is that audit is necessary and not sufficient. Implementation-class risk in this family is real and is the reason §16 keeps the TSS-selection decision open.
...
- Code-base churn: the active branch as of mid-2026 is `cggmp24`, post the CVE-2025-66017 fixes. Depending on the audited `cggmp21` branch means freezing on a known-but-patched vintage. Depending on `cggmp24` means re-auditing the API-changed successor.
...
- Validator-set milestone math from §13.3 binds. Any growth past $n \approx 100$ in a regime where CGGMP21 has been promoted from fallback to primary forces a hard look at epoch-rotation cadence. Tens-of-seconds DKG at every epoch boundary across every foreign-chain vault is a meaningful operator-side load that does not show up in single-corridor benchmarks.
