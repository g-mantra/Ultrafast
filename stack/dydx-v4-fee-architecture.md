...
1. **5% validator-commission floor** (§13.1, §13.5). UltraFast's working assumption is a commission floor of 5% and a ceiling of 20% on the bonded-delegator share. The 5% floor is taken directly from dYdX v4, where validator commission rates have a hard-coded minimum of 5% (against a 100% maximum). UltraFast cites this in two places: once in the fee-distribution table (§13.1) and again in the anti-concentration measures (§13.5). The same floor serves two functions. It prevents commission undercutting races to zero (which would push small delegators toward the largest validators). It also guarantees validators a non-zero margin on operations cost.

2. **100% of liquidation penalties to the insurance fund** (§13.1). UltraFast routes liquidation revenue to the IF separately from trading fees, "matching dYdX, Drift, and Aevo industry practice." dYdX v4's insurance fund is funded by liquidation penalties (default 1.5% maximum liquidation penalty, governance-tunable). ADL only triggers when the IF is exhausted. UltraFast follows the same shape: no trading-fee carve-out for the IF. The backstop relies on liquidation throughput plus the §10.5 dispute-window-and-kill-switch policy.

3. **Fee tiers by 30-day rolling volume** (§13.1, parameter list). dYdX v4's fee schedule scales taker fees down from ~5 bps at Tier 1 to ~2.5 bps at Tier 6. Maker rebates scale up based on trailing 30-day USD volume across all perpetual books. The top tiers are gated by market-share thresholds in addition to volume. UltraFast adopts the same breakpoint structure as the working default for governance to tune.
...
The citation pattern in the whitepaper is therefore narrow and defensible. UltraFast takes two specific parameter choices (commission floor, IF funding source) and one schedule shape (30-day volume tiers) from dYdX v4. It does not inherit dYdX v4's consensus, matching, custody, or fee-currency decisions.
...
- **Distribution flow:** Each block, fees accrue to the `fee_collector` module account, transfer to the `x/distribution` module on the next block, then:
  ...
  - `validator_commission` is subtracted per validator (5% to 100% range, set per-validator).
...
UltraFast's three explicit borrowings from dYdX v4 (the 5% commission floor, 100%-of-liquidation-penalties-to-IF, and 30-day-volume fee tiering) are well-supported by the dYdX v4 production record. The departures (BTC vs USDC fee currency, no `community_tax` carve-out, FBA vs continuous matching, MCP-based MEV resistance, TSS+ZK custody, greenfield Rust stack) are deliberate. They are motivated by the criticisms of dYdX v4 itself: the validator-orderbook MEV surface, the USDC-bridge single-asset dependency, and the reflexivity of token-denominated rewards. The §15 framing ("follows the dYdX-v4 precedent in two specific elements") accurately describes the relationship.
